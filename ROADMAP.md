...
- **Markdown parser replaced with markdown-it (full / minimal modes)** — the hand-rolled regex pipeline is gone; rendering now uses markdown-it with two configurations selected by a new per-board `Board.markdown_enabled` setting (migration `0050`, defaults dynamically to `SiteSettings.allow_markdown` at board-creation time): full mode (`default` preset + custom quote-link plugin) for markdown-enabled boards, minimal mode (`zero` preset + greentext + quote-links + line breaks + paragraphs) for the rest. Greentext is deliberately excluded from full mode to preserve CommonMark blockquote semantics. `MarkdownBody.jsx` rewritten around DOMPurify with `ADD_ATTR: ['target']` (DOMPurify strips `target="_blank"` silently even when `rel` is set — caught by live testing)
- **Thread OP editing (parity with reply editing)** — `Thread` gains `edited_at` and `edit_count`; `PATCH /api/threads/<id>/edit/` on `ThreadViewSet` respecting the existing `allow_post_editing` and `post_edit_window_seconds` site settings; `ThreadDetail.jsx` gets the same edit UI as replies — countdown timer, edited badge
- **Redirect to new thread after posting** — `BoardDetail.jsx` and `CommunityDetail.jsx` navigate straight to `/thread/<id>` after thread creation instead of refreshing the board list, so the edit-window countdown is immediately visible
- **Fixed: board mappings stuck permanently "Pending"** — the ActivityPub Accept handler read `accepted_obj.get('object')` (the remote's own actor URL) instead of `accepted_obj.get('actor')` (our local actor URL), so the lookup against the local `Actor` table could never match and `follow_accepted` never flipped to `True`. Verified end-to-end with a live simulation using the real `build_follow_activity`/`build_accept_activity` utilities
- **Fixed: federated replies never triggered bell notifications** — `thread.reply_count` is only incremented in `core/views.py`, never via a signal, so `federation/inbound.py`'s `handle_create_reply` silently bypassed the entire notification pipeline — breaking even the 60-second polling fallback, not just WebSocket push. Fixed by adding the `reply_count` increment, `FeedItem` bulk-create for watchers, and `channel_layer.group_send` to the inbound handler
- **Replaced `dperson/torproxy` with `peterdavehello/tor-socks-proxy`** — the old image ships a multi-year-stale Tor binary (0.4.3.5) with a long-standing, still-open bug that permanently sticks at "Bootstrapped 30%: Loading networkstatus consensus" (dperson/torproxy issues #35/#47/#87), confirmed by direct comparison against the inbound `tor` container (newer Tor, bootstraps in seconds). The replacement is actively maintained (automated Tor version bumps via Renovate) and SOCKS5-only, which is all `FEDERATION_SOCKS_PROXY` needs. **Note: the SOCKS port changed from 9050 to 9150** — the compose default and deployment docs are updated accordingly. Tor state is persisted across restarts, and the image's built-in healthcheck (which depended on Facebook's onion staying up) is disabled
...
- **Federation Phase 6 — Tor SOCKS outbound** — per-URL transport routing in `fetch.py` (`_client_for`): `.onion` destinations route through a dedicated outbound Tor container (`tor-proxy` — originally `dperson/torproxy` on port 9050, since replaced by `peterdavehello/tor-socks-proxy` on port 9150, see the later replacement entry) via `FEDERATION_SOCKS_PROXY`, clearnet goes direct; `FEDERATION_SOCKS_PROXY` setting; `socksio` dependency; longer timeout for onion circuits; `fetch_instance_boards` task implemented (was referenced but missing); wired into dev test stack and prod compose
